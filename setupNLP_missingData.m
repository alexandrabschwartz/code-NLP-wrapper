function [problem, n_x, n_lin, n_nln] = setupNLP_missingData(problem)

% This function is given a nonlinear optimization problem of the form
%    min f(x)  s.t. xl <=   x  <= xu
%                   bl <=  A*x <= bu
%                   cl <= c(x) <= cu.
...
% In case therea re no linear or nonlinaer constraints, a dummy constraint 
% of this type with n_... = 0 is inserted to simplify the later reformulation
% of the NLP.
...
elseif length(problem.x_start) == 1
    disp('Warning: Only one value for x_start provided, using this for all variables')
